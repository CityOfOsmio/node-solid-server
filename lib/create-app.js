...
const SimpleAccountManager = require('./models/simple-account-manager')
...
function initWebId (argv, app, ldp) {
  ...
  /** TODO:
   * Temporary shim: SimpleAccountManager allows for delaying the requirement of a WebID URI
   * until after the initial certificate exchange
   * For now, we will simply "derive" an accountName from the provided certificate from the S/N in crockford base32
  **/
  var accountManager
  let accountManagerOptions = {
    ...
    caDomain: app.locals.caDomain
  }
  if  (argv.auth == 'mtls') {
    accountManager = SimpleAccountManager.from(accountManagerOptions)
  } else {
    accountManager = AccountManager.from(accountManagerOptions)
  }
