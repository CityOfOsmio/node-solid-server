...
class MtlsAuthenticator extends TlsAuthenticator {
  ...
  findValidUser () {
    return this.renegotiateTls()
      ...
      .then(cert => this.deriveWebId(cert))
   ...
   * @return {Promise<string>} WebID URI
   */
  deriveWebId (certificate) {
    return new Promise((resolve, reject) => {
      if (!certificate.serialNumber) {
        return reject("Peer Certificate has no serial number")
      }

      let username = API.authn["mtls"].deriveWebId(certificate)
      debug("deriveWebId: [" + username + "]")
      resolve(this.accountManager.accountWebIdFor(username))
    }).catch((e) => {
      var error = 'Error deriving WebID from certificate'
      debug(error, e)
      return reject(error)
    })
  }
