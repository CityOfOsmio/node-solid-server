...
/**
 * Does basic "mutual TLS" user authentication based on configured Root CA certificate,
 * so that simple client-side authentication consists of validating the presented certificate as
 * signed by our chosen CA.
 *
 * Key function here is findValidUser(), which uses a lot of WebID assumptions. We need to override and
 * replace these, until such time as we can embrace and extend them.
 */
class MtlsAuthenticator extends TlsAuthenticator {
  /**
   * @constructor
   * @param options {Object}
   *
  ...
  static fromParams (req, options) {
    options.connection = req.connection
    options.caDomain = req.app.locals.caDomain

    return new MtlsAuthenticator(options)
  }
  ...
  findValidUser () {
    return this.renegotiateTls()
      ...
      .then(cert => this.getWebId(cert))

      .then(webId => this.loadUser(webId))
  }

  /**
   * Derives the WebID URI from a client certificate.
   *
   * @param certificate {X509Certificate}
   *
   * @return {Promise<string>} WebID URI
   */
  getWebId (certificate) {
    return new Promise((resolve, reject) => {
      if (!certificate.serialNumber) {
        return reject("Peer Certificate has no serial number")
      }

      try {

        //let webid = API.authn['mtls'].deriveWebId(certificate, this.caDomain)
        let webid = this.accountManager.deriveWebIdFor(certificate, this.caDomain)
        debug("getWebId: [" + webid + "]")

      } catch(e) {
        var error = 'Error deriving WebID from certificate in getWebId'
        debug(error, e)
        return Promise.reject(error)
      }

      resolve(webid)
      // TODO: convert this to a local account "webid" ?
      // this gets a local WebId, i think?
      //resolve(this.accountManager.accountWebIdFor(username))
    })
  }
}
