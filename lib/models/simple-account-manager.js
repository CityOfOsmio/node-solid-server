...
debug("simple-account-manager API")
debug(API)
...
class SimpleAccountManager extends AccountManager {
  ...
  userAccountFrom (userData) {
    ...
    debug(userData)
    var webid = this.deriveWebIdFor(userData)
  ...
  /**
   * Derive a webID for the external caDomain and certificate given.
   *
   * @param certificate
   * @returns {*}
   */
  deriveWebIdFor(certificate) {
    debug("deriveWebIdFor")
    var webid = API.authn["mtls"].deriveWebId(certificate, this.caDomain)
    debug(webid)
    return webid
  }
}
