...
function createServer (argv, app) {
  ...
  if (!needsTLS) {
    ...
    if (argv.ca && argv.auth == "mtls") {
      debug.settings('SSL CA Certificate path: ' + argv.ca);

      try {
        credentials.ca = []
        argv.ca.forEach(function (file, index) {
          credentials.ca.push(fs.readFileSync(file));
        });
      } catch (e) {
        throw new Error('Can\'t find CA cert in ' + argv.ca)
      }
    }

    if ((ldp.webid && ldp.auth === 'tls') || ldp.auth == 'mtls') {
