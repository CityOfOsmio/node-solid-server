...
function handler (req, res, next) {
  ...
  try {
    ...
    debug('Error deriving WebID from certificate in mtls API', e)
    setEmptySession(req)
    return next()
  }
...
function deriveWebIdUsername(certificate) {
  if (certificate.serialNumber) {
    debug("Serial Number: ["+certificate.serialNumber+"]")
    let asc = new Buffer(certificate.serialNumber, 'hex')
    debug(asc);
    return base32.encode(asc)
  } else {
