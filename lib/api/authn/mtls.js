...
function handler (req, res, next) {
  ...
  try {
    ...
    req.session.userId = req.app.locals.accountManager.accountWebIdFor(deriveWebId(certificate))
    res.set('User', req.session.userId)
    debug("mtls deriveWebId: ["+req.session.userId+"]")
 ...
 */
function deriveWebId(certificate) {
